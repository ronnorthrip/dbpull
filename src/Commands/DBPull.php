...
class DBPull extends Command
{
    ...
    {
        if (App::environment('production')) { $this->error('This server IS PRODUCTION. So NOPE.'); return self::INVALID; }
        if (!$this->setup()) { $this->error('A required env setting or config is missing.'); return self::INVALID; }
        if (!$this->ssh_ping()) { $this->error('Could not connect to remote server via ssh'); return self::FAILURE; }
        if ($this->option('ping')) { $this->info('PING! Connect to remote server via ssh worked.'); return self::SUCCESS; }
        $full_dump = $this->option('full-dump');
        $replace = $full_dump || $this->option('replace');
        if ($replace) {
            if (!$this->confirm('Are you sure you want to REPLACE the local data?')) { return self::SUCCESS; }
        }
        $force = $full_dump || $this->option('force');
        if (!$force) {
            if ($this->ssh_count_pending_migrations() > 0) {
                $this->error('Production app has pending migrations. Please apply migrations before proceeding.'); return self::INVALID;
            }
            if ($this->local_count_pending_migrations() > 0) {
                $this->error('Local app has pending migrations. Please apply migrations before proceeding.'); return self::INVALID;
            }
            if ($this->ssh_count_migrations() !== $this->local_count_migrations()) {
                $this->error('Local and remote migrations do not match. Data structure differences wont work here'); return self::INVALID;
            }
        }
        ...
        if ($tables) {
            ...
            if ($missing) {
                $this->error('Missing '.count($missing).' table(s) from remote database. '.implode(', ', $missing)); return self::INVALID;
            }
            $missing = array_diff($tables, $this->local_get_tables());
            if ($missing) {
                $this->error('Missing '.count($missing).' table(s) from local database. '.implode(', ', $missing)); return self::INVALID;
            }
        }
        ...
        foreach ($tables as $table) {
            if (!in_array($table, $skip_tables)) {
                if ($replace) {
                    $open_tables[$table] = 0;
                } else if (Schema::hasColumn($table, 'id')) {
    ...
    public function ssh_table_dump_updated_id_rows($table, $justcount) {
        if (!$this->has_old_data()) { return; }
        if ($this->option('skip-updates')) { return; }
    ...
    public function ssh_table_dump_deleted_id_rows($table, $max) {
        $max = $max;
        $count = 0;
        if ($this->option('skip-deletes')) { return; }
        $select =
    ...
    public function local_count_pending_migrations() {
        ...
        Artisan::call('migrate:status', array(), $output);
    ...
    public function has_old_data() {
        ...
        if (array_key_exists($from, $this->data)) { return true; }
        return false;
    }

    public function get_data($table, $col) {
        $from = $this->from_lc;
        if (!array_key_exists($from, $this->data)) { return null; }
        if (!array_key_exists($table, $this->data[$from])) {  return null; }
        if (!array_key_exists($col, $this->data[$from][$table])) {  return null; }
    ...
    public function set_data($table, $col, $val) {
        $from = $this->from_lc;
        if (!array_key_exists($from, $this->data)) {  $this->data[$from] = []; }
        if (!array_key_exists($table, $this->data[$from])) {  $this->data[$from][$table] = []; }
    ...
    public function move_old_data() {
        $from = $this->from_lc;
        if (!array_key_exists($from, $this->data)) { return; }
